...
int screenWidthInit = 0;
int screenHeightInit = 0;
...
bool open_audio_complete = false;
...
static int stream_component_open(VideoState *is, int stream_index) {
    ...
    switch (avctx->codec_type) {
        ...
        case AVMEDIA_TYPE_SUBTITLE:
            ...
            if ((ret = decoder_init(&is->subdec, avctx, &is->subtitleq, is->continue_read_thread)) <
                0)
                goto fail;
...
static int read_thread(void *arg) {
    ...
    if (err < 0) {
        LOGE("%s, %s", is->filename, av_err2str(err));
        ret = -1;
        goto fail;
    }
    ...
    if (find_stream_info) {
        ...
        if (err < 0) {
            LOGE("Error setting up avformat_find_stream_info() options\n");
            ret = err;
            goto fail;
        }
...
static int video_open(VideoState *is) {
    ...
    w = screen_width ? screen_width : screenWidthInit;
    h = screen_height ? screen_height : screenHeightInit;
...
int SDL_main(int argc, char *argv[]) {
    ...
    char *heightScreenChar = argv[argc - 1];
    char *widthScreenChar = argv[argc - 2];

    int defaultWidth = atoi(widthScreenChar);
    int defaultHeight = atoi(heightScreenChar);

    screenWidthInit = defaultWidth;
    screenHeightInit = defaultHeight;
    ...
    if (!display_disable) {
        ...
        window = SDL_CreateWindow("ffplay", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                                  defaultWidth, defaultHeight, flags);
